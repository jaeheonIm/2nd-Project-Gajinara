...
<div class="container">
    ...
    <div class="container">
		<h3>안녕하세요 ${mvo.id } 님</h3><br>
		<h4>글수정 페이지 입니다</h4>

		<div class="row">
        <div class="col-lg-8 col-md-7 order-md-1 order-1" style="flex: 100%; max-width: 100%">
			<div id="board-list">
            <form name="firstForm" action="${pageContext.request.contextPath}/boardcontrol?type=modifyView&ok=ok"method="post" enctype="multipart/form-data">
				<table class="table">
                <tbody>
				<tr>
                    <td width="20%">
						<select name="category" id="category">
                        <option label="text" selected>${bvo.category }</option>
                        <c:forEach var="cvo" items="${caList }">
							<option value="${cvo.value }">${cvo.value }</option>
                        </c:forEach>
						</select>
                    </td>
                    <td><input type="button" value="글수정" onclick="modify_go()" class="site-btn" style="float: right"/></td>
				</tr>
				...
				<tr>
                    ...
                    <td>
					<input type="text" value="${bvo.title }" id="title" style="width: -webkit-fill-available" name="title"/>
                    </td>
				</tr>
				<tr>
                    ...
                    <td>
					<textarea name="content" style="width: 100%; height: 20em">${bvo.content }</textarea>
                    </td>
				</tr>
                <tr>
                    ...
                    <td>
                    <input type="text" name="price" value="${bvo.price }" id="price"  style="width: 100%"/>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="3">
						<div class="row" id="thumbBox">							
							...
							<div class="col-lg-3">
                            <img id="${img.key }" src="${pageContext.request.contextPath}/upload/${img.value }" />                           		
                        	</div> 
                            </c:forEach>							
						</div>
                    </td>
